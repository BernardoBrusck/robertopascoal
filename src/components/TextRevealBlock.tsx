...
const TextRevealBlock = ({ text, className = '' }: TextRevealBlockProps) => {
  ...
  return (
    <section ref={sectionRef} className={`relative w-full bg-background flex items-center justify-center py-32 md:py-48 px-6 md:px-16 lg:px-24 ${className}`}>
      <p ref={textRef} className="text-reveal-text text-reveal-text--sm font-sans max-w-5xl text-center leading-[1.3]">{text}</p>
    </section>
  );
};
